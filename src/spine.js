...
import { Black, Sprite, DisplayObject } from 'black-engine';
...
export class Spine extends DisplayObject {
  ...
  onUpdate() {
    ...
    for (let i = 0, n = slots.length; i < n; i++) {
      ...
      if (sprite)
        sprite.mSuspendDirty = true;

      if (wrapper)
        wrapper.mSuspendDirty = true;
      ...
      if (sprite) {
        sprite.mSuspendDirty = false;
        sprite.setTransformDirty();
      }

      if (wrapper) {
        wrapper.mSuspendDirty = false;
        wrapper.setTransformDirty();
      }
    }
  }
